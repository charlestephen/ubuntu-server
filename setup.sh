...
docker run -d --name elk --restart=always -v /${CACHE_MOUNTPOINT}/logstash:/opt/logstash -v /${CACHE_MOUNTPOINT}/kibana:/opt/kibana -v /${CACHE_MOUNTPOINT}/elasticsearch:/var/lib/elasticsearch -p 5601:5601 -p 9200:9200 -p 5044:5044 -e PGID=${DOCKER_USER} -e PUID=${DOCKER_USER}  -e ${LOCAL_TIMEZONE_CODE} sebp/elk

#Emby Media Manager
docker run -d --name emby --restart=always -v /${CACHE_MOUNTPOINT}/emby:/config -v /${STORAGE_MOUNTPOINT}/Media:/mnt/media -p 8096:8096 -p 8920:8920 -e UID=1000 -e GID=999 -e ${LOCAL_TIMEZONE_CODE} -e GIDLIST=44 emby/embyserver:latest
...
docker run -d --name homebridge --restart=always --net=host -e PUID=${DOCKER_USER} -e PGID=${DOCKER_USER} -e ${LOCAL_TIMEZONE_CODE} -v /${CACHE_MOUNTPOINT}/homebridge:/homebridge oznu/homebridge
...
#Observium
docker run -d --name observium --restart=always -p 8668:8668 -e ${LOCAL_TIMEZONE_CODE} -v /${CACHE_MOUNTPOINT}/observium/config:/config -v /${CACHE_MOUNTPOINT}/observium/logs:/opt/observium/logs -v /${CACHE_MOUNTPOINT}/observium/rrd:/opt/observium/rrd zuhkov/observium
...
docker run -d --name pihole --restart=always -p 53:53/tcp -p 53:53/udp -p 8082:80 -v /${CACHE_MOUNTPOINT}/pihole:/etc/pihole -v /${CACHE_MOUNTPOINT}/pihole/dnsmasq.d:/etc/dnsmasq.d -e ServerIP=${CIDR} -e ServerIPv6=${CIDR} -e ${LOCAL_TIMEZONE_CODE} --restart=always diginc/pi-hole:alpine
